...
func SeedInitialBuildings(db *gorm.DB, svc *services.ContentService) error {
	buildings := []models.BuildingDefinition{
		...
		// More from reference (full 20 from doc; samples shown, expand with all effects from reference)
		{
		...
		// Add the remaining 10+ from the full reference (BLD-009 to BLD-020) with exact data from doc: costs, effects, dependencies, IA actions, palier visuals.
		// Example for BLD-020:
		{
		...
		// Repeat pattern for all 20 using the tables in the reference doc (costs, per level effects, IA slots, risks, actions, visual tiers).
	}
